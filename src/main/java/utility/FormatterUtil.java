...
public class FormatterUtil {
  ...
  public static String formatForDisplay(BigDecimal v, int maxDigits) {
    ...
    // BigDecimal formattedDecimals =  decimalsRemovedExtraZeros.round(new MathContext(maxDigits, RoundingMode.HALF_UP));

    // formattedは小数点以下の文字数しか制限できなかったので、整数＋小数点以下が８文字以下になるように手動で余剰分をカットする
    StringBuilder sb = new StringBuilder();
    if((integerCount + decimalsRemovedExtraZeros) >= maxDigits) {
      for(int i = 0; i <= 8; i ++) {
        sb.append(連結後.toString().charAt(i));
      }
    }
