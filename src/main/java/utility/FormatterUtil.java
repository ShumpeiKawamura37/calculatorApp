...
public class FormatterUtil {
  ...
  public static String formatForDisplay(BigDecimal v, int maxDigits) {
    ...
    // 少数部分の余剰な0を排除
    // BigDecimal decimalsRemovedExtraZeros = new BigDecimal(decimals.toString()).stripTrailingZeros();

    // 少数部分の余剰な0を排除し、8桁でまとめる。
    return v.stripTrailingZeros().round(new MathContext(maxDigits, RoundingMode.HALF_UP)).toPlainString();

    // decimalsRemovedExtraZerosとintegersを連結させて、roundする
    // BigDecimal linked = new BigDecimal(integers.append(decimalsRemovedExtraZeros).toString());
